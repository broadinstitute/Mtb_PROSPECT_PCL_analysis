...
loocv_num_workers_parfor = 4 % number of workers to use in parallel for spectral clustering step; original results used a parfor loop with 4 physical CPU cores
...
if prepare_loocv
    ...
    for i = index_cmpds_loocv
        ...
        % Create a cluster object for the 'local' profile
        cluster = parcluster('local');

        % Set NumWorkers to 4
        cluster.NumWorkers = loocv_num_workers_parfor;

        % Save the modified profile
        cluster.saveProfile;

        % Start a parallel pool with 4 workers
        parpool(cluster, loocv_num_workers_parfor);
        ...
        parfor ii = 1:numel(moas)
            ...
            % Initialize 'c' and 'cr' as empty structures
            c = struct();
            cr = struct();
            ...
            try
                ...
                disp(sprintf('Error: %s correlation gctx file not found', moa_class))
                disp(ME)
                continue; % Skip to the next iteration since 'c' is invalid
            end
            ...
            try
                ...
                disp(sprintf('Error: %s correlation rank gctx file not found', moa_class))
                disp(ME)
                continue; % Skip to the next iteration since 'cr' is invalid
            end
            
            % Check if 'cr' is valid before proceeding
            if isempty(cr) || ~isfield(cr, 'rid')
                disp(sprintf('Warning: Invalid "cr" data for MOA class %s', moa_class));
        ...
        % Shut down the parallel pool when done
        delete(gcp('nocreate'));
